...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"../lib/p5.play.js":[function(require,module,exports) {
var define;
...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) define('p5.play', ['p5'], function (p5) {
    factory(p5);
  ...
  */
  ...
  p5.prototype.updateSprites = function (upd) {
    ...
    if (this.spriteUpdate) for (var i = 0; i < this.allSprites.size(); i++) {
  ...
  */
  ...
  p5.prototype._isKeyInState = function (key, state) {
    ...
    if (keyStates[keyCode] === undefined) {
      if (this.keyIsDown(keyCode)) keyStates[keyCode] = KEY_IS_DOWN;else keyStates[keyCode] = KEY_IS_UP;
    }
  ...
  */
  ...
  p5.prototype._isMouseButtonInState = function (buttonCode, state) {
    ...
    if (buttonCode === undefined) buttonCode = this.LEFT; //undefined = not tracked yet, start tracking

    if (mouseStates[buttonCode] === undefined) {
      if (this.mouseIsPressed && this.mouseButton === buttonCode) mouseStates[buttonCode] = KEY_IS_DOWN;else mouseStates[buttonCode] = KEY_IS_UP;
    }
      ...
      if (this.keyIsDown(key)) //if is down
        {
          if (keyStates[key] === KEY_IS_UP) //and was up
            keyStates[key] = KEY_WENT_DOWN;else keyStates[key] = KEY_IS_DOWN; //now is simply down
        } else //if it's up
        {
          if (keyStates[key] === KEY_IS_DOWN) //and was up
            keyStates[key] = KEY_WENT_UP;else keyStates[key] = KEY_IS_UP; //now is simply down
        }
    } //mouse
      ...
      if (this.mouseIsPressed && this.mouseButton === btn) //if is down
        {
          if (mouseStates[btn] === KEY_IS_UP) //and was up
            mouseStates[btn] = KEY_WENT_DOWN;else mouseStates[btn] = KEY_IS_DOWN; //now is simply down
        } else //if it's up
        {
          if (mouseStates[btn] === KEY_IS_DOWN) //and was up
            mouseStates[btn] = KEY_WENT_UP;else mouseStates[btn] = KEY_IS_UP; //now is simply down
        }
    }
  };
  /**
  ...
  */
  ...
  p5.prototype.useQuadTree = function (use) {
    if (this.quadTree !== undefined) {
      if (use === undefined) return this.quadTree.active;else if (use) this.quadTree.active = true;else this.quadTree.active = false;
    } else return false;
  }; //the actual quadTree
  ...
  */
  ...
  function Sprite(pInst, _x, _y, _w, _h) {
    ...
    var camera = pInst.camera; // These are p5 constants that we'd like easy access to.
    ...
    */
    ...
    this.newPosition = createVector(_x, _y); //Position displacement on the x coordinate since the last update
    ...
    */
    ...
    this.collider = undefined; //internal use
    ...
    */
    ...
    this.immovable = false; //Coefficient of restitution - velocity lost in the bouncing
    ...
    */
    ...
    if (_w === undefined) this.width = 100;else this.width = _w;
    /**
    * Height of the sprite's current image.
    * If no images or animations are set it's the height of the
    * placeholder rectangle.
    ...
    */
    ...
    if (_h === undefined) this.height = 100;else this.height = _h;
    /**
    ...
    */
    ...
    this.groups = [];
    var animations = {}; //The current animation's label.
    ...
    */
    ...
    this._syncAnimationSizes = function () {
      ...
      if (this.colliderType === 'default' && animations[currentAnimation].getWidth() !== 1 && animations[currentAnimation].getHeight() !== 1) {
        ...
        this._internalHeight = animations[currentAnimation].getHeight() * abs(this._getScaleY()); //quadTree.insert(this);
    ...
    */
    ...
    this.update = function () {
      if (!this.removed) {
        ...
        if (this.newPosition !== this.position) this.previousPosition = createVector(this.newPosition.x, this.newPosition.y);else this.previousPosition = createVector(this.position.x, this.position.y);
        ...
        this.deltaY = this.position.y - this.previousPosition.y; //if there is an animation
        ...
        if (this.mouseActive) {
          ...
          if (typeof this.onMouseOver === 'function' || typeof this.onMouseOut === 'function' || typeof this.onMousePressed === 'function' || typeof this.onMouseReleased === 'function') {
            ...
            this.mouseActive = true; //if no collider set it
    ...
    this.setDefaultCollider = function () {
      ...
      if (animations[currentAnimation] && animations[currentAnimation].getWidth() !== 1 && animations[currentAnimation].getHeight() !== 1) {
        ...
        this.colliderType = 'image'; //print("IMAGE COLLIDER ADDED");
      } else if (animations[currentAnimation] && animations[currentAnimation].getWidth() === 1 && animations[currentAnimation].getHeight() === 1) {//animation is still loading
        //print("wait");
      } else //get the with and height defined at the creation
        {
          this.collider = new AABB(pInst, this.position, createVector(this._internalWidth, this._internalHeight)); //quadTree.insert(this);
    ...
    this.mouseUpdate = function () {
      ...
      this.mouseIsPressed = false;
      var mousePosition;
      if (camera.active) mousePosition = createVector(camera.mouseX, camera.mouseY);else mousePosition = createVector(pInst.mouseX, pInst.mouseY); //rollover

      if (this.collider) {
        if (this.collider instanceof CircleCollider) {
          if (dist(mousePosition.x, mousePosition.y, this.collider.center.x, this.collider.center.y) < this.collider.radius) this.mouseIsOver = true;
        ...
        if (this.mouseIsOver && pInst.mouseIsPressed) this.mouseIsPressed = true; //event change - call functions

        if (!mouseWasOver && this.mouseIsOver && this.onMouseOver !== undefined) if (typeof this.onMouseOver === 'function') this.onMouseOver.call(this, this);else print('Warning: onMouseOver should be a function');
        if (mouseWasOver && !this.mouseIsOver && this.onMouseOut !== undefined) if (typeof this.onMouseOut === 'function') this.onMouseOut.call(this, this);else print('Warning: onMouseOut should be a function');
        if (!mouseWasPressed && this.mouseIsPressed && this.onMousePressed !== undefined) if (typeof this.onMousePressed === 'function') this.onMousePressed.call(this, this);else print('Warning: onMousePressed should be a function');
        if (mouseWasPressed && !pInst.mouseIsPressed && !this.mouseIsPressed && this.onMouseReleased !== undefined) if (typeof this.onMouseReleased === 'function') this.onMouseReleased.call(this, this);else print('Warning: onMouseReleased should be a function');
      }
    };
    /**
    ...
    */
    ...
    this.getBoundingBox = function () {
      ...
      var h = animations[currentAnimation].getHeight() * abs(this._getScaleY()); //if the bounding box is 1x1 the image is not loaded
    ...
    */
    ...
    this.mirrorX = function (dir) {
      if (dir === 1 || dir === -1) dirX = dir;else return dirX;
    };
    /**
    ...
    */
    ...
    this.mirrorY = function (dir) {
      if (dir === 1 || dir === -1) dirY = dir;else return dirY;
    };
    /*
    ...
    this.display = function () {
      if (this.visible && !this.removed) {
        ...
        this.draw(); //draw debug info

        pop();

        if (this.debug) {
          push(); //draw the anchor point
    ...
    */
    ...
    this.remove = function () {
      this.removed = true;
      quadTree.removeObject(this); //when removed from the "scene" also remove all the references in all the groups
    ...
    */
    ...
    this.getDirection = function () {
      ...
      if (isNaN(direction)) direction = 0; // Unlike Math.atan2, the atan2 method above will return degrees if
    ...
    */
    ...
    this.addToGroup = function (group) {
      if (group instanceof Array) group.add(this);else print('addToGroup error: ' + group + ' is not a group');
    };
    /**
    ...
    */
    ...
    this.addImage = function () {
      if (typeof arguments[0] === 'string' && arguments[1] instanceof p5.Image) this.addAnimation(arguments[0], arguments[1]);else if (arguments[0] instanceof p5.Image) this.addAnimation('normal', arguments[0]);else throw 'addImage error: allowed usages are <image> or <label>, <image>';
    };
    /**
    ...
    */
    ...
    this.changeAnimation = function (label) {
      if (!animations[label]) print('changeAnimation error: no animation labeled ' + label);else {
        currentAnimation = label;
    ...
    */
    ...
    this.overlapPixel = function (pointX, pointY) {
      ...
      var img = this.animation.getFrameImage(); //convert point to img relative position

      point.x -= this.position.x - img.width / 2;
      point.y -= this.position.y - img.height / 2; //out of the image entirely

      if (point.x < 0 || point.x > img.width || point.y < 0 || point.y > img.height) return false;else if (this.rotation === 0 && this.scale === 1) {
        //true if full opacity
        var values = img.get(point.x, point.y);
        return values[3] === 255;
      } else {
        print('Error: overlapPixel doesn\'t work with scaled or rotated sprites yet'); //offscreen printing to be implemented bleurch
    ...
    */
    ...
    this.overlapPoint = function (pointX, pointY) {
      ...
      if (this.collider !== undefined) {
        if (this.collider instanceof AABB) return point.x > this.collider.left() && point.x < this.collider.right() && point.y > this.collider.top() && point.y < this.collider.bottom();
    ...
    */
    ...
    this.AABBops = function (type, target, callback) {
      ...
      this.touching.top = false;
      this.touching.bottom = false;
      var result = false; //if single sprite turn into array anyway

      var others = [];
      if (target instanceof Sprite) others.push(target);else if (target instanceof Array) {
        ...
        if (this !== others[i] && !this.removed) //you can check collisions within the same group but not on itself
          {
            ...
            if (this.collider !== undefined && other.collider !== undefined) {
              if (type === 'overlap') {
                var over; //if the other is a circle I calculate the displacement from here
                ...
                displacement = createVector(0, 0); //if the sum of the speed is more than the collider i may
                ...
                if (tunnelX || tunnelY) {
                  ...
                  var c = createVector((this.position.x + this.previousPosition.x) / 2, (this.position.y + this.previousPosition.y) / 2); //the extents are the distance between the coll centers
                  ...
                  var bbox = new AABB(pInst, c, e, this.collider.offset); //bbox.draw();
  ...
  defineLazyP5Property('Camera', boundConstructorFactory(Camera)); //called pre draw by default

  function cameraPush() {
    var pInst = this;
    var camera = pInst.camera; //awkward but necessary in order to have the camera at the center
  ...
  p5.prototype.Group = Group; //circle collider - used internally
  ...
  function CircleCollider(pInst, _center, _radius, _offset) {
    ...
    if (_offset === undefined) this.offset = createVector(0, 0);else this.offset = _offset;
  ...
  defineLazyP5Property('CircleCollider', boundConstructorFactory(CircleCollider)); //axis aligned bounding box - extents are the half sizes - used internally
    ...
    this.overlap = function (other) {
      ...
      if (other instanceof AABB) {
        ...
        if (md.min().x <= 0 && md.max().x >= 0 && md.min().y <= 0 && md.max().y >= 0) {
          ...
          var pt = createVector(other.center.x, other.center.y); //I don't know what's going o try to trace a line from centers to see

          if (other.center.x < this.left()) pt.x = this.left();else if (other.center.x > this.right()) pt.x = this.right();
          if (other.center.y < this.top()) pt.y = this.top();else if (other.center.y > this.bottom()) pt.y = this.bottom();
          ...
          if (distance < other.radius) {
            ...
            if (pt.x === other.center.x && pt.y === other.center.y) {
              ...
              if (abs(xOverlap) < abs(yOverlap)) {
                if (xOverlap > 0) pt.x = this.right();else pt.x = this.left();
              } else {
                if (yOverlap < 0) pt.y = this.top();else pt.y = this.bottom();
              }

              a = pInst.atan2(other.center.y - pt.y, other.center.x - pt.x); //fix exceptions
              ...
              a = pInst.atan2(pt.y - other.center.y, pt.x - other.center.x); //project the normal (line between pt and center) onto the circle
            }

            var d = createVector(pt.x - other.center.x, pt.y - other.center.y);
            var displacement = createVector(pInst.cos(a) * other.radius - d.x, pInst.sin(a) * other.radius - d.y); //if(pt.x === other.center.x && pt.y === other.center.y)
            //displacement = displacement.mult(-1);

            return displacement; //return createVector(0,0);
          } else return createVector(0, 0);
        }
    };
  ...
  function Animation(pInst) {
    ...
    */
    ...
    this.frameChanged = false; //is the collider defined manually or defined
    //by the current frame size

    this.imageCollider = false; //sequence mode

    if (frameArguments.length === 2 && typeof frameArguments[0] === 'string' && typeof frameArguments[1] === 'string') {
      var from = frameArguments[0];
      var to = frameArguments[1]; //print("sequence mode "+from+" -> "+to);
      ...
      if (from !== -1 && to !== -1) {
        ...
        var prefix2 = to.substring(0, to.length - (4 + digits2)); // Our numbers likely have leading zeroes, which means that some
        ...
        var number2 = parseInt(to.substring(to.length - (4 + digits2), to.length - 4), 10); //swap if inverted
    ...
    // Sprite sheet mode
      ...
      } else if (frameArguments.length !== 0) //arbitrary list of images
        {
          ...
          for (i = 0; i < frameArguments.length; i++) {
            ...
            if (frameArguments[i] instanceof p5.Image) this.images.push(frameArguments[i]);else this.images.push(pInst.loadImage(frameArguments[i]));
          }
        }
    /**
    ...
    */
    ...
    this.draw = function (x, y, r) {
      ...
      if (this.visible) {
        ...
        if (!this.isSpriteAnimation) this.update(); //this.currentImageMode = g.imageMode;
        ...
        if (this.images[frame] !== undefined) {
          ...
          pInst.print('Warning undefined frame ' + frame); //this.isActive = false;
        }
        ...
        if (targetFrame > frame && targetFrame !== -1) {
          frame++;
          ...
          } else if (this.looping) //advance frame
            {
              ...
              if (frame >= this.images.length - 1) frame = 0;else frame++;
            } else {
            //if next frame is too high
    ...
    */
    ...
    this.changeFrame = function (f) {
      if (f < this.images.length) frame = f;else frame = this.images.length - 1;
      targetFrame = -1; //this.playing = false;
    };
    /**
    ...
    */
    ...
    this.nextFrame = function () {
      if (frame < this.images.length - 1) frame = frame + 1;else if (this.looping) frame = 0;
      targetFrame = -1;
      this.playing = false;
    };
    /**
    ...
    */
    ...
    this.previousFrame = function () {
      if (frame > 0) frame = frame - 1;else if (this.looping) frame = this.images.length - 1;
      targetFrame = -1;
      this.playing = false;
    };
    /**
  ...
  defineLazyP5Property('SpriteSheet', boundConstructorFactory(SpriteSheet)); //general constructor to be able to feed arguments as array
  ...
  */
  ...
  Quadtree.prototype.getIndex = function (pRect) {
    if (!pRect.collider) return -1;else {
      var index = -1,
      ...
      bottomQuadrant = pRect.collider.top() > horizontalMidpoint; //pRect can completely fit within the left quadrants
  ...
  Quadtree.prototype.getObjectNode = function (obj) {
    ...
    if (!this.nodes.length) {
      ...
      index = this.getIndex(obj); //if the object does not fit into a subnode, it must be here
  ...
  p5.prototype.registerMethod('pre', p5.prototype.readPresses); //automatic sprite update

  p5.prototype.registerMethod('pre', p5.prototype.updateSprites); //quadtree update

  p5.prototype.registerMethod('post', updateTree); //camera push and pop

  p5.prototype.registerMethod('pre', cameraPush);
  p5.prototype.registerMethod('post', cameraPop); //deltaTime
...
},{}]},{},["../../../../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","../lib/p5.play.js"], null)
//# sourceMappingURL=/p5.play.895723a0.js.map